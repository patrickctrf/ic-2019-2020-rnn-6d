...
def get_model_functional(neurons, batch_size, X):
    ...
    output = LSTM(neurons, batch_input_shape=(batch_size, X.shape[1], X.shape[2]), stateful=True)(concatenate([input, output]))
    output = Dense(1)(output)
...
Runs the experiment itself.
    ...
    for r in range(repeats):
        # fit the base model
        # lstm_model, training_history = fit_lstm(train=train_scaled, batch_size=1, nb_epoch=500, neurons=3, validation_data=test_scaled);
