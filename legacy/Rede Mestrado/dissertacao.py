...
neuroniosCamada1 = 250
neuroniosCamada2 = 250
neuroniosCamada3 = 250
neuroniosCamada4 = 250
...
# Print model
plot_model(model, to_file='modelMestrado.png', show_shapes=True, dpi=150)

# Xfinal = repeteMatrizDataset(Xt, Xt_1, 4)

# fit model
history = model.fit([Xt[1:int(len(Xt)/2)], Xt[1:int(len(Xt)/2)], Xt[1:int(len(Xt)/2)], Xt[1:int(len(Xt)/2)], Xt_1[1:int(len(Xt)/2)], Xt_1[1:int(len(Xt)/2)], Xt_1[1:int(len(Xt)/2)], Xt_1[1:int(len(Xt)/2)]], [Yt[1:int(len(Xt)/2)], Yt_1[1:int(len(Xt)/2)]], epochs=15)
...
# json_file = open('modelMestrado.json', 'r')
# loaded_model_json = json_file.read()
# json_file.close()
# loaded_model = model_from_json(loaded_model_json)
# # load weights into new model
# loaded_model.load_weights("modelMestrado.h5")
...
json_file = open("modelMestrado.json", "w")
json_file.write(model_json)
json_file.close()
model.save_weights("modelMestrado.h5")
print("Model saved to disk")

# Print model
plot_model(model, to_file='modelMestrado.png', show_shapes=True)
...
for i in range(int(Xt.shape[0]/2)):
    ...
    for j in range(Xt.shape[1]):
        ...
        for k in range(Yt.shape[1]):
            Xt_1[i + int(Xt.shape[0]/2)][j][k + int(Xt.shape[2])] = Ytrajetoria[i][k]
